...
def calculate_data(data,freq=None):
        # 通过data计算特征
        """===================时域特征========================="""
        # 峰值
        peak_value = data["Vertical_acceleration"].max()
        # 均方根值
        rms_value = np.sqrt(np.mean(data["Vertical_acceleration"] ** 2))
        # 方差
        variance = np.var(data["Vertical_acceleration"])
        # 整流平均值
        rectified_mean = np.mean(np.abs(data["Vertical_acceleration"]))
        # 峰峰值
        peak_to_peak_value = (
            data["Vertical_acceleration"].max()
            - data["Vertical_acceleration"].min()
        )
        # 方根幅值
        rmsa = np.mean(np.sqrt(np.abs(data["Vertical_acceleration"]))) ** 2
        # 峭度
        kurtosis_value = data["Vertical_acceleration"].kurt()
        # 偏度
        skewness_value = data["Vertical_acceleration"].skew()
        # 波形因子
        ...
        # 能量
        energy = np.sum(data["Vertical_acceleration"] ** 2)
        ...
        fs = freq if freq else (1 / (data["time"][1] - data["time"][0]).total_seconds())
        # 计算信号的长度
        n = len(data.loc[:, "Vertical_acceleration"])

        # 计算频率
        f = np.linspace(0, fs, n)

        # 计算频谱
        fft = np.fft.fft(data["Vertical_acceleration"])
...
class Bearing_Dataset(Dataset):
    def __init__(self, bearing_path):
        ...
        for index, subfile in enumerate(subfiles):
            ...
            with open(subfile_name, "r") as data_file:
                ...
                # 删去原有的时间数据，只保留时间戳    time + Horizontal_acceleration + Vertical_acceleration
                bearing_data = bearing_data.drop(
                    ["hour", "minute", "second", "micro","Horizontal_acceleration"], axis=1#不要水平的震动数据
                )
                bearing_data["rul"] = index / self.length  #time + Horizontal_acceleration + Vertical_acceleration + rul
    ...
    def __getitem__(self, idx):
        ...
        for sub_bearing_data in bearing_data_seq:
            bearing_data = self.calculate_data(sub_bearing_data)   #时域+频域特征
            # print(bearing_data)
...
def bearing_dataloader(bearing_path, batch_size, num_workers=8):
    ...
    # 划分训练集和测试集
    train_size = int(0.8 * len(bearing_dataset)) #0.8
    test_size = len(bearing_dataset) - train_size  #0.2
    ...
    if use_epoch:
        ...
        for step in range(total_steps):
            ...
            if step + 1 % save_stpes == 0 and best_state_dict is not None:
                torch.save(model.state_dict(), "bearing_predictor_v.pth")
                print(
                    "-----------------------------------------------------------------------------------------------------------------------"
                )
...
def main():
    ...
    # 保存模型
    torch.save(model, "bearing_predictor_v.pth")
